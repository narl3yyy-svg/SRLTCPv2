...
Using SRLTCP v0.2.31 for secure peer-to-peer messaging.
...
1. Download `SRLTCPv2-0.2.31.apk` from [Releases](https://github.com/narl3yyy-svg/SRLTCPv2/releases/latest) or build locally (see [BUILD.md](BUILD.md))
...
SRLTCP v0.2.31 uses **QR v4** with an **iroh ticket** for NAT traversal — no port forwarding or WAN settings required. Paste the peer's QR and tap **Connect & Verify**.
...
| Android app stuck on loading | Update to **v0.2.31+** (UI shows before engine init; work runs on IO thread) |
| SAS confirm does nothing / crash | Both peers on **v0.2.31+**; initiator (who pasted QR) confirms first |
...
| Transfer stuck | Both peers on **v0.2.31+**; cancel and retry; reconnect saved contact if connection dropped mid-transfer |
| Sender sees "received" file bubble | Update to **v0.2.31+** on both peers |
| Reconnect to saved contact fails | **v0.2.31+** tears down stale sessions automatically; ensure peer is online with a recent QR saved |
| Android back shows SAS screen | **v0.2.31+** — back dismisses SAS/sheets; swipe again moves app to background |
| Call has no audio / hangup breaks UI | **v0.2.31+**; desktop: recv-only if no mic; Android: grant mic in Settings |
| No serial ports listed | Plug in device, click **Refresh**; Linux: add user to `dialout` group |
| Serial connect fails | Both peers on v0.2.31+; try another baud rate; check cable |
