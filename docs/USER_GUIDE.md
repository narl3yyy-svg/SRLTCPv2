...
Using SRLTCP v0.3.2 for secure peer-to-peer messaging.
...
1. Download `SRLTCPv2-0.3.2.apk` from [Releases](https://github.com/narl3yyy-svg/SRLTCPv2/releases/latest) or build locally (see [BUILD.md](BUILD.md)). Default APK is **64-bit ARM** (most modern phones).
...
SRLTCP uses **QR v4** with an **iroh ticket** for NAT traversal — no port forwarding or WAN settings required. Paste the peer's QR and tap **Connect & Verify**. Your identity is saved on this device so contacts keep working after restart.
...
| Android app stuck on loading | Update to **v0.3.2+** (UI shows before engine init; work runs on IO thread) |
| SAS confirm does nothing / crash | Both peers on **v0.3.2+**; initiator (who pasted QR) confirms first |
| Contacts lost after restart | v0.3.2+ persists identity seed; reinstalling the app resets identity — re-verify SAS |
...
| Transfer stuck | Both peers on **v0.3.2+**; cancel and retry; reconnect saved contact if connection dropped mid-transfer |
| Sender sees "received" file bubble | Update to **v0.3.2+** on both peers |
| Reconnect to saved contact fails | **v0.3.2+** tears down stale sessions automatically; ensure peer is online with a recent QR saved |
| Android back shows SAS screen | **v0.3.2+** — back dismisses SAS/sheets; swipe again moves app to background |
| Call has no audio / hangup breaks UI | **v0.3.2+**; desktop: recv-only if no mic; Android: grant mic in Settings |
| No serial ports listed | Plug in device, click **Refresh**; Linux: add user to `dialout` group |
| Serial connect fails | Both peers on v0.3.2+; try another baud rate; check cable |
