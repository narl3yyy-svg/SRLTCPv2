...
impl P2pEngine {
    ...
    /// Connect using endpoint embedded in peer QR (if needed), then run SAS handshake.
    pub async fn connect_and_verify(&self, remote_qr: &str) -> Result<(String, String), String> {
        ...
        let mut peer_id = peers.first().cloned();
        ...
        let peer_id = peer_id.ok_or_else(|| {
            "Could not reach peer. Ensure both apps are running on the same network, \
             and use a QR code generated by a running SRLTCP app (v0.2.6+ includes the address)."
                .to_string()
        })?;
